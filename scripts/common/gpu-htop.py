...
# USAGE
#
# [amd/nvidia-smi | ] gpu-htop.py [-l [length]] [-i ID]
...
parser.add_argument('-t', '--type', default='amd-smi', type=str)
...
if fake_stdin_path is not None:
    ...
    lines = stdin_lines
    if len(args.id) > 0:
        print('amd/nvidia-htop argument -i/--id cannot be used when amd/nvidia-smi output is being piped into it. To filter the'
              ' shown GPUs, pass the -i argument to the amd/nvidia-smi call instead.', file=sys.stderr)
else:
    cli_args = []
    if len(args.id) > 0:
        cli_args = ['-i', args.id]
    ps_call = subprocess.run([args.type] + cli_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if ps_call.returncode != 0:
        print('smi exited with error code {}:'.format(ps_call.returncode), file=sys.stderr)
...
def colorize(_lines):
    ...
    # Index of the first content line of the current cell in the amd/nvidia-smi output.
    start_idx = 0
